...
const SpecialTraining = ({studySpecialData, loading}) => {
    ...
    return (
      <li key={index}>
        ...
        <p className={styles.title} title={item.SeriesName}>
          <Link to="/main/specialOne" target="_blank" rel="noopener noreferrer">{item.SeriesName}</Link>
        </p>
      </li>
    )
  });
  return (
    <div className={styles.specialTraining}>
      <GeneralHead showIcon={true} title="专题学习" url="/main/specialLearning"></GeneralHead>
