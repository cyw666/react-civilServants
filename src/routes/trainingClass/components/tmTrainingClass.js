  ...
  const classList = ListData.map((item, index) => {
    return (
      <Row key={ item.Id } className={ styles.tabList }>
        ...
        <Col span={ 6 }><p
          className="list">{ dateFilter(item.StartTime, 'yyyy-MM-dd') + '~' + dateFilter(item.EndTime, 'yyyy-MM-dd') }</p>
        </Col>
        {
          item.ApplyStatus === 'UnApprove' ?
            ...
            item.ApplyStatus === 'Normal' ?
              <Col span={ 4 }><p className="list"><Button type={ 'primary' }><Link
                to={ { pathname: "/main/grade/classDetail", search: `?id=${item.Id}` } }
                rel="noopener noreferrer">点击进入</Link></Button></p></Col> :
    ...
    <div className={ styles.tmTrainingClass }>
      ...
      <div className={ styles.classList }>
        ...
        <Spin spinning={ loading }>
          <Tabs activeKey={ classType } onChange={ (key) => {
            getClass(key)
          } }>
