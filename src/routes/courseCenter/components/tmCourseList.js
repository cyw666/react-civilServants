    ...
    return (
      <tr key={index}>
        ...
        <td className={styles.playRight}>
          ...
          <p className={styles.play}><Button type={'primary'}><Link to="courseDetail">点击播放</Link></Button></p>
        </td>
      </tr>
    )
  })
    ...
    <div className={styles.tmCourseList}>
      ...
      <div className={styles.courseBorder}>
        <Breadcrumb>
          ...
          <Breadcrumb.Item>课程中心</Breadcrumb.Item>
        </Breadcrumb>
        <div className="searchText">
          <Search
            placeholder="请输入要搜索的课程"
            style={{width: '75%'}}
            onSearch={value => {onSearchCourse({title:value,page:1})}}
          />
        </div>
        <div className={styles.sortCourse}>
          <div className={styles.tag}>
            <span><img src={sfp}/><a onClick={()=>{searchType('ThreeScreenCourse')}}> 精品三分屏</a></span>
            <span><img src={dsp}/><a onClick={()=>{searchType('SingleCourse')}}> 单视频课件</a></span>
            <span><img src={dh}/><a onClick={()=>{searchType('AnimationCourse')}}> 动画类课件</a></span>
          </div>
          <div className={styles.titleBar}>
            <CourseOrder onChange={(activeKey,order,sort)=>{onSearchCourse({page:1,sort,order})}}>
              {searchList}
            </CourseOrder>
          </div>
        </div>
        <div className={styles.courseContent}>
          ...
          <Row>
            <Col span={24}>
              ...
              <Button onClick={()=>{onAddStudyCourse(checkedList)}}>批量选课</Button>
              <Pagination showQuickJumper defaultCurrent={1} current={pageConfig.current} pageSize={pageConfig.pageSize} total={pageConfig.total} onChange={(currentPage)=>{onSearchCourse({page:currentPage})}} showTotal={(total)=>{return `共${total}条`}}/>
            </Col>
          </Row>
        </div>
      </div>
