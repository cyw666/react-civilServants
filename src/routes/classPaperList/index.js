...
const ClassPaperList = ({ classPaperList, dispatch, loading }) => {
  ...
  const contentList = classPaperListData.ListData && classPaperListData.ListData.map((item, index) => {
    return (
      <Row key={ item.Id }>
        <Col span={ 12 } className="ellipsis"><Link
          to={ { pathname: '/main/articleDetail', search: `?id=${item.Id}&type=class` } } title={ item.Name }
    ...
    <div className={ styles.classPaperList }>
      ...
      <div className={ styles.innerContent }>
        <Breadcrumb>
          ...
          } }>{ classPaperListData.TrainingName }</Link></Breadcrumb.Item>
