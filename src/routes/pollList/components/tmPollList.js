...
const TmPollList = ({ pollListData, pollParams, inputSearch, loading, pageConfig }) => {
  const { Model: { UnfinishModel } } = pollListData;
  const pollList = UnfinishModel && UnfinishModel.map((item, index) => {
    return (
      <tr key={ item.Id }>
        <td>
          <Link to={ { pathname: "/poll", search: `?id=${item.Id}` } } title={ item.Name }
                target="_blank" rel="noopener noreferrer">
            { item.Name }
          </Link>
        </td>
        <td>
          <Link className="tableName" to={ { pathname: "/poll", search: `?id=${item.Id}` } } title={ item.Name }
                target="_blank" rel="noopener noreferrer">
            参加
          </Link>
        </td>
