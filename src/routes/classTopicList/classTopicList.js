...
const ClassTopicList = ({classTopicList, dispatch, loading}) => {
  ...
  const contentList = classTopicListData.ListData && classTopicListData.ListData.map((item, index) => {
    return (
      <Row key={index}>
        <Col span={8}  className="ellipsis"><Link to={`/main/articleDetail/${item.Id}`} title={item.Name} target="_blank" rel="noopener noreferrer">{item.Name}</Link></Col>
    ...
    <div className={styles.classTopicList}>
      ...
      <div className={styles.innerContent}>
        <Breadcrumb>
          ...
          <Breadcrumb.Item><Link to={{pathname: '/main/grade/classDetail', query: {id: classId}}}>{classTopicListData.TrainingName}</Link></Breadcrumb.Item>
