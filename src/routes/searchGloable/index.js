...
const SearchGloable = ({ searchGloable, dispatch, loading }) => {
  ...
  const restuleList = resultData.map((item, index) => {
    if (item.Type === 'Article') {
      return (
        <li key={ item.Id }>
          <Link className="tableName" to={ { pathname: "/main/articleDetail", search: `?id=${item.Id}` } }
    ...
    if (item.Type === 'Notice') {
      return (
        <li key={ item.Id }>
          <Link className="tableName" to={ { pathname: "/main/noticeDetail", search: `?id=${item.Id}` } }
                title={ item.Name }
                target="_blank"
                rel="noopener noreferrer">
            [通知：]{ item.Name }
    ...
    return (
      <li key={ item.Id }>
        <Link className="tableName" to={ { pathname: "/main/courseDetail", search: `?id=${ item.Id }` } }
              title={ item.Name }
