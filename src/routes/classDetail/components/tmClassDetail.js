...
const TmClassDetail = ({detailData, loading}) => {
  ...
  const noticeList = ClassNoticeList&&ClassNoticeList.map((item,index)=>{
    return (
      <Row key={index} className={styles.tabList}>
        <Col span={12}><div className="list">{item.Name}</div></Col>
        <Col span={6}><div className="list">{dateFilter(item.CreatedDate, 'yyyy-MM-dd')}</div></Col>
        <Col span={4}><div className="list">{item.Author}</div></Col>
      </Row>
    )
  });
  const requiredCourseList = ClassRequiredCourseList&&ClassRequiredCourseList.map((item,index)=>{
    return (
      <Row key={index} className={styles.tabList}>
        <Col span={12}><div className="list">{item.Name}</div></Col>
        <Col span={6}><div className="list"><Progress percent={parseFloat(item.BrowseScore.toFixed(1))} status="active"/></div></Col>
        <Col span={4}><div className="list"><Link to={{pathname:'/play',query:{courseId:item.Id}}} target="_blank" rel="noopener noreferrer">点击播放</Link></div></Col>
      </Row>
    )
  });
  ...
  const examList = ClassExamList&&ClassExamList.map((item,index)=>{
    return (
      <Row key={index} className={styles.tabList}>
        <Col span={9}><div className="list">{item.Name}</div></Col>
        <Col span={5}><div className="list">{item.Credit}</div></Col>
        <Col span={5}><div className="list">{item.TotalScore}</div></Col>
        <Col span={5}><div className="list"><Link to={{pathname:'/main/exam',query:{id:item.Id}}} target="_blank" rel="noopener noreferrer">参加测试</Link></div></Col>
      </Row>
    )
  });
  const topicList = ClassTopicList&&ClassTopicList.map((item,index)=>{
    return (
      <Row key={index} className={styles.tabList}>
        ...
        <Col span={5}><div className="list">{item.Type}</div></Col>
        <Col span={5}><div className="list">{dateFilter(item.CreatedDate, 'yyyy-MM-dd')}</div></Col>
        <Col span={5}><div className="list">{item.Author}</div></Col>
      </Row>
    )
  });
  const paperList = ClassPaperList&&ClassPaperList.map((item,index)=>{
    return (
      <Row key={index} className={styles.tabList}>
        <Col span={12}>
          <div className="list">
            <Link to={`/main/articleDetail/${item.Id}`}  title={item.Name} target="_blank" rel="noopener noreferrer">{item.Name}</Link>
          </div>
        </Col>
        <Col span={6}><div className="list">{item.Author}</div></Col>
        <Col span={4}><div className="list">{dateFilter(item.CreatedDate, 'yyyy-MM-dd')}</div></Col>
      </Row>
    )
  });
        ...
        <Spin spinning={loading}>
          <div className={styles.classDes}>
            ...
            <div className={styles.publish}>
              <span>我要发表：</span>
              <Link to={{pathname: '/main/grade/classTopicAdd', query: {id: 5}}}><img src={topic} alt=""/></Link>
              <Link to={{pathname: '/main/grade/classPaperAdd', query: {id: 5}}}><img src={paper} alt=""/></Link>
              <Link to={{pathname: '/main/grade/photoAlbumAdd', query: {id: 5}}}><img src={album} alt=""/></Link>
            </div>
            ...
            <Tabs defaultKey={`classNotice`} onChange={(key) => {
              ...
              <TabPane tab="班级公告" key="classNotice">
                <Row className={styles.title}>
                  <Col span={12}><div className="list">公告</div></Col>
                  <Col span={6}><div className="list">时间</div></Col>
                  <Col span={4}><div className="list">作者</div></Col>
                </Row>
                {noticeList}
              </TabPane>
              <TabPane tab="必修课程" key="requiredCourse">
                <Row className={styles.title}>
                  <Col span={12}><div className="list">班级名称</div></Col>
                  <Col span={6}><div className="list">进度</div></Col>
                  <Col span={4}><div className="list">播放</div></Col>
                </Row>
                {requiredCourseList}
              </TabPane>
              ...
              <TabPane tab="班级考试" key="classExam">
                <Row className={styles.title}>
                  ...
                  <Col span={5}><div className="list">考试学时</div></Col>
                  <Col span={5}><div className="list">最高分</div></Col>
                  <Col span={5}><div className="list">参加测试</div></Col>
                </Row>
                {examList}
              </TabPane>
              <TabPane tab="班级话题" key="classTopic">
                <Row className={styles.title}>
                  <Col span={9}><div className="list">话题</div></Col>
                  <Col span={5}><div className="list">话题分类</div></Col>
                  <Col span={5}><div className="list">时间</div></Col>
                  <Col span={5}><div className="list">作者</div></Col>
                </Row>
                {topicList}
              </TabPane>
              <TabPane tab="班级论文" key="classPaper">
                <Row className={styles.title}>
                  <Col span={12}><div className="list">论文标题</div></Col>
                  <Col span={6}><div className="list">作者</div></Col>
                  <Col span={4}><div className="list">创建时间</div></Col>
                </Row>
                {paperList}
              </TabPane>
