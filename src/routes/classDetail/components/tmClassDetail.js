  ...
  const requiredCourseList = ClassRequiredCourseList && ClassRequiredCourseList.map((item, index) => {
    return (
      <Row key={ item.Id } className={ styles.tabList }>
        ...
        <Col span={ 4 }>
          <div className="list"><Link to={ { pathname: '/play', search: `?id=${item.Id}` } } target="_blank"
  ...
  const examList = ClassExamList && ClassExamList.map((item, index) => {
    return (
      <Row key={ item.Id } className={ styles.tabList }>
        ...
        <Col span={ 5 }>
          <div className="list"><Link to={ { pathname: '/exam', search: `?id=${item.Id}` } } target="_blank"
  ...
  const topicList = ClassTopicList && ClassTopicList.map((item, index) => {
    return (
      <Row key={ item.Id } className={ styles.tabList }>
        <Col span={ 9 }>
          <div className="list">
            <Link to={ { pathname: '/main/articleDetail', search: `?id=${item.Id}&type=class` } } target="_blank"
  ...
  const paperList = ClassPaperList && ClassPaperList.map((item, index) => {
    return (
      <Row key={ item.Id } className={ styles.tabList }>
        <Col span={ 12 }>
          <div className="list">
            <Link to={ { pathname: '/main/articleDetail', search: `?id=${item.Id}&type=class` } } title={ item.Name }
                  target="_blank"
                  rel="noopener noreferrer">{ item.Name }</Link>
          </div>
        </Col>
  ...
  const photoList = ClassPhotoList && ClassPhotoList.map((item, index) => {
    return (
      <Col key={ item.Id } span={ 6 }>
        <Link to={ { pathname: '/main/grade/photoPreview', search: `?albumId=${item.Id}&id=${Id}` } }>
        ...
        <Spin spinning={ loading }>
          <div className={ styles.classDes }>
            ...
            <div className={ styles.publish }>
              <span>我要发表：</span>
              <Link to={ { pathname: '/main/classTopicAdd', search: `?id=5` } }><img src={ topic } alt=""/></Link>
              <Link to={ { pathname: '/main/classPaperAdd', search: `?id=5` } }><img src={ paper } alt=""/></Link>
              <Link to={ { pathname: '/main/photoAlbumAdd', search: `?id=5` } }><img src={ album } alt=""/></Link>
            </div>
