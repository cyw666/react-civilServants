...
import { login, LoginCode, getVerifyCode, kickOut, userMessage } from '../services/'
    ...
    setup({ dispatch, history }) {
      history.listen(({ pathname, search }) => {
        if (pathname === '/main/login') {
          dispatch({ type: 'setTitle', payload: { title: '登陆' } });
          dispatch({ type: 'updateState', payload: queryString.parse(search) });
